import { getOfflineLibraryTracks } from "../offline/mobileOfflineRepository";
...
function getTrackFileName(track) {
  ...
  if (relativePath) {
    ...
    return relativePathParts[relativePathParts.length - 1] || `offline-${track.id}`;
  }
...
function hasMatchingExtension(track, extensionFilter) {
  ...
  const normalizedExtension = extensionFilter.trim().toLowerCase().replace(/^\./, "");
...
function sortTracks(tracks, sortBy = "title", order = "asc") {
  ...
  return [...tracks].sort((left, right) => {
    ...
    const comparedValue = String(leftValue).localeCompare(String(rightValue), undefined, {
      sensitivity: "base",
      numeric: true,
    });
