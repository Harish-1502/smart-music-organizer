...
describe("downloadLibrary", () => {
  beforeEach(() => {
    ...
    mobileOfflineRepositoryMocks.ensureMobileOfflineDbReady.mockResolvedValue(true);
  ...
  it("fetches backend tracks only in LAN mode and skips already verified downloads", async () => {
    ...
    expect(mobileOfflineRepositoryMocks.getBulkOfflineTrackVerification).toHaveBeenCalledWith(
      ["track-1", "track-2"],
    );
  ...
  it("allows a fresh retry after a cancelled run with a new abort state", async () => {
    ...
    expect(offlineTrackDownloadMocks.downloadTrackForOffline).toHaveBeenCalledTimes(3);
  });
  ...
  it("uses the bulk verification helper instead of per-track lookup helpers", async () => {
    ...
    mobileOfflineRepositoryMocks.getBulkOfflineTrackVerification.mockResolvedValue(
      new Map([
        ["track-1", { verified: true, hasTrackRow: true, hasAudioRef: true, sizeBytes: 1 }],
        ["track-2", { verified: false, hasTrackRow: false, hasAudioRef: false, sizeBytes: 0 }],
      ]),
    );
    ...
    expect(mobileOfflineRepositoryMocks.getBulkOfflineTrackVerification).toHaveBeenCalledTimes(1);
  });
  ...
  it("uses the verification map to classify broken local refs for retry", async () => {
    ...
    mobileOfflineRepositoryMocks.getBulkOfflineTrackVerification.mockResolvedValue(
      new Map([
        ["track-1", { verified: true, hasTrackRow: true, hasAudioRef: true, brokenLocalRef: false, sizeBytes: 10 }],
        ["track-2", { verified: false, hasTrackRow: true, hasAudioRef: true, brokenLocalRef: true, sizeBytes: 0 }],
        ["track-3", { verified: false, hasTrackRow: false, hasAudioRef: false, brokenLocalRef: false, sizeBytes: 0 }],
      ]),
    );
  ...
  it("uses native SQLite metadata persistence on Android without storing raw PC paths", async () => {
    ...
    expect(result.downloadedCount).toBe(1);
    expect(mobileOfflineRepositoryMocks.ensureMobileOfflineDbReady).toHaveBeenCalledTimes(2);
    expect(mobileOfflineRepositoryMocks.saveOfflineTrackWithMediaRefs).toHaveBeenCalledWith({
      id: "track-1",
      title: "Native Save",
      artist: "Artist A",
      album: "Album A",
    ...
    expect(mobileOfflineRepositoryMocks.saveOfflineTrackWithMediaRefs).not.toHaveBeenCalledWith(
      expect.objectContaining({
        file_path: "S:\\Music\\native-save.mp3",
      }),
    );
  ...
  it("initializes the mobile offline database before the first Android track download", async () => {
    ...
    const callOrder = [];
    mobileOfflineRepositoryMocks.ensureMobileOfflineDbReady.mockImplementationOnce(async () => {
  ...
  it("allows a clean retry after a failed run", async () => {
    ...
    expect(offlineTrackDownloadMocks.downloadTrackForOffline).toHaveBeenCalledTimes(2);
  });
  ...
  it("keeps one shared full-library job active across duplicate start attempts", async () => {
    ...
    resolveDownload();
    const [result, duplicateResult] = await Promise.all([firstPromise, secondPromise]);
  ...
  it("retries database initialization on a new run after a startup database failure", async () => {
    ...
    expect(retryResult.downloadedCount).toBe(1);
    expect(offlineTrackDownloadMocks.downloadTrackForOffline).toHaveBeenCalledTimes(1);
    expect(mobileOfflineRepositoryMocks.ensureMobileOfflineDbReady).toHaveBeenCalledTimes(3);
  });
  ...
  it("returns a true empty-library result only when the backend has zero tracks", async () => {
    ...
    expect(mobileOfflineRepositoryMocks.ensureMobileOfflineDbReady).not.toHaveBeenCalled();
    expect(offlineTrackDownloadMocks.downloadTrackForOffline).not.toHaveBeenCalled();
  });
  ...
  it("treats abort errors as a cancelled job instead of a failed retry", async () => {
    ...
    offlineTrackDownloadMocks.downloadTrackForOffline.mockRejectedValueOnce(abortError);
  ...
  it("cleans up newly written native files when track metadata persistence fails", async () => {
    ...
    expect(console.error.mock.calls[0][0]).toContain("[full-library-download:error]");
    expect(console.error.mock.calls[0][0]).not.toContain("[object Object]");
    expect(console.error.mock.calls[0][0]).toContain("\"operation\": \"saving-track-metadata\"");
  });
  ...
  it("continues downloading later tracks after one metadata save failure", async () => {
    ...
    mobileOfflineRepositoryMocks.getBulkOfflineTrackVerification.mockResolvedValue(
      new Map([
        ["track-1", { verified: false, hasTrackRow: false, hasAudioRef: false, sizeBytes: 0 }],
        ["track-2", { verified: false, hasTrackRow: false, hasAudioRef: false, sizeBytes: 0 }],
      ]),
    );
    offlineTrackDownloadMocks.downloadTrackForOffline
