...
function sanitizeSafeErrorMessage(error, fallback = "A track could not be downloaded.") {
    ...
    `[full-library-download:error] ${JSON.stringify(
      {
        operation,
        trackId:
          track?.id ?? track?.track_id ?? track?.trackId ?? error?.trackId ?? null,
        trackTitle:
    ...
    for (const track of missingTracks) {
      ...
      try {
        ...
        result = await downloadTrackForOffline(track, {
          ...
          existingTrackState:
            verificationMap.get(normalizeTrackId(track)) ?? {
              verified: false,
              existingTrack: null,
              sizeBytes: 0,
            },
