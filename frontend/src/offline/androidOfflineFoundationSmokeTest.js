...
export async function runAndroidOfflineFoundationSmokeTest({
  ...
  if (!allowInProduction && typeof import.meta !== "undefined" && !import.meta.env?.DEV) {
  ...
  try {
    ...
    if (sqliteSteps.initialize) {
      ...
      sqliteSteps.writeMetadata = Boolean(savedTrack && savedPlaylist && savedOrder);
    ...
    if (isNativeMediaFileStorageSupported()) {
      ...
      try {
        ...
        if (!fileSteps.readUri) {
          captureFileFailure("getUri", new Error("Native media URI was not found."), savedFile?.relativePath ?? expectedTestAudioPath);
  ...
  logger.info?.("[offline-smoke] sqlite init:", sqliteSteps.initialize ? "pass" : "fail");
  logger.info?.(
    "[offline-smoke] sqlite write/read/clear:",
    sqliteSteps.writeMetadata && sqliteSteps.readMetadata && sqliteSteps.clearMetadata
      ? "pass"
      : "fail",
  );
  logger.info?.(
  ...
  if (fileFailure.step) {
    logger.info?.("[offline-smoke] native media failed step:", fileFailure.step);
    logger.info?.("[offline-smoke] native media error:", `${fileFailure.errorName}: ${fileFailure.errorMessage}`);
    if (fileFailure.relativePath) {
      logger.info?.("[offline-smoke] native media safe path:", fileFailure.relativePath);
    }
  }
