...
export default function useTrackEdit({ loadTracks, setMessage }) {
  ...
  const selectedTrackArtPath = selectedTrackId
    ? getTrackArtPath(selectedTrackId)
    : "";
  const { blobUrl: remoteArtUrl } = useAuthenticatedBlobUrl(
    selectedTrackArtPath,
    {
  ...
  async function handleCreateTag() {
    ...
    try {
      ...
      try {
        const attachedTag = await addTagToTrack(
          selectedTrack.id,
          createdTag.id,
        );
        setTrackTags((prev) => [...prev, attachedTag]);
        ...
        setTagsError(
          error.message ||
            "Tag created, but failed to attach it to this track.",
        );
      }
    } catch (error) {
